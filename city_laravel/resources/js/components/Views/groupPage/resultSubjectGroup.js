...
const ResultSubjectGroup = ({ idForFetch, showAllData, limitsForyLabels , nameItemClicked }) => {
    ...
    async function fetchData() {
        setisLoaded(true)
        try {
            const result = await fetchApi(`http://127.0.0.1:8000/grade/${idForFetch}`)
            console.log("TCL: fetchData -> result", result)
            setJsonApi(result)

            const gamesPlayedInfo = await fetchApi(`http://127.0.0.1:8000/gamesPlayed/grade/${idForFetch}`)
...
export default ResultSubjectGroup