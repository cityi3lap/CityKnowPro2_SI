...
const ResultSubjectInstitution = ({ idForFetch, showAllData, limitsForyLabels, nameItemClicked }) => {
    ...
    async function fetchData() {
        setisLoaded(true)
        try {
            const result = await fetchApi(`http://127.0.0.1:8000/institution/${idForFetch}`)
            setJsonApi(result)

            const gamesPlayedInfo = await fetchApi(`http://127.0.0.1:8000/gamesPlayed/institution/${idForFetch}`)
...
export default ResultSubjectInstitution;