...
class GetByHeadquarter extends Controller
{
    /**
     * Receives a headquarter id and returns the average of score for every subject segmented by 
     * headquarter. First looks for the info in Redis, if that's not found then looks for the info in 
     * the database and saves it in Redis.
     */
    protected function get($id) {
        // if (Redis::get('headquarter-'.$id)) {
        //     $result = json_decode(Redis::get('headquarter-'.$id));
        // } else {
            $result = DB::table('game_user_records')
                        ...
                        ->where('game_users.headquarter_id', $id)
                        ->groupBy('subjects.name')
                        ->get();

        //     !empty($result[0]) ? Redis::set('headquarter-'.$id, $result->toJson()) : null;
        // }
    ...
    protected function getIntelligences($id) {
    ...
    protected function getStyles($id) {
        $result = DB::table('gu_record_intelligence_ind_desc_styles AS guri')
                    ->join('game_user_records', 'guri.game_user_record_id', '=', 'game_user_records.id')
                    ->join('game_users', 'game_users.id', '=', 'game_user_records.game_user_id')
    ...
    protected function getIntelligencesByCompetence($id) {
        $result = DB::table('gu_record_intelligence_ind_desc_styles AS guri')
                    ->join('game_user_records', 'guri.game_user_record_id', '=', 'game_user_records.id')
                    ->join('game_users', 'game_users.id', '=', 'game_user_records.game_user_id')
                    ->join('intelligence_indicators AS intin', 'intin.id', '=', 'guri.intelligence_indicator_id')
    ...
    protected function getStylesByCompetence($id) {
        $result = DB::table('gu_record_intelligence_ind_desc_styles AS guri')
                    ->join('game_user_records', 'guri.game_user_record_id', '=', 'game_user_records.id')
                    ->join('game_users', 'game_users.id', '=', 'game_user_records.game_user_id')
                    ->join('description_styles AS descSt', 'descSt.id', '=', 'guri.description_style_id')
    ...
    protected function getScoreByMGGrade($id) {
    ...
    protected function getVocationals($id) {
        $result = DB::table('gu_record_intelligence_ind_desc_styles AS guri')
                    ->join('game_user_records', 'guri.game_user_record_id', '=', 'game_user_records.id')
                    ->join('game_users', 'game_users.id', '=', 'game_user_records.game_user_id')
    ...
    protected function getRecomendationsAndDbas($id) {
        ...
        foreach ($groupBySunAndGrade as $key_sg => $element) {
            foreach ($element as $key => $value) {
                ...
                $result = DB::table('performances AS per')
                        ...
                        ->where('rec.hierarchy_id', 4) // 4 is the id for headquarters in hierarchies
                        ->get();
