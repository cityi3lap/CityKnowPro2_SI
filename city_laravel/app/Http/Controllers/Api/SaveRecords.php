...
class SaveRecords extends Controller
{
    ...
    protected function save(Request $request) {
        ...
        $last_avatar_change = DB::table('avatar_changes')
                                ...
                                ->select('avatars.id', 'avatars.skin', 'avatars.profession', 
                                    'avatars.gender', 'avatars.age', 'avatar_changes.avatar_name')
        ...
        if ($avatars !== null && isset($avatars[0])) {
            foreach ($avatars as $key => $avatar) {
                ...
                if (isset($avatar->skin) && isset($avatar->gender) && isset($avatar->proffesion)
                    && isset($avatar->name)) {
                    if (!$last_avatar_change || ($last_avatar_change->skin !== $avatar->skin || 
                        $last_avatar_change->gender !== $avatar->gender ||
                        $last_avatar_change->profession !== $avatar->proffesion ||
                        ...
                        if($avatar_db) {
                            ...
                            if ($avatar->server['lastdate'] == '' && $avatar->server['lastdate'] == null) {
                                $avatars[$key]['server'] = (object) array('status' => 1, 
        ...
        if ($minigames !== null && isset($minigames[0])) {
            foreach ($minigames as $key=>$minigame) {
                ...
                if ($minigameById !== null) {
                    ...
                    if (isset($progress) && isset($progress->errors) && isset($progress->repeatedGuide)
                        ...
                        || $minigame->server['lastdate'] == null)) {
                            ...
                            $minigames[$key]['server'] = (object) array('status' => 1, 
                                                                        'lastdate' => date("Y-m-d H:i:s"), 
                                                                        'platform' => '');
                    }
                }
                    ...
                    if (isset($progress) && isset($progress->errors) && isset($progress->repeatedGuide)
                        ...
                        && ($minigame->server['lastdate'] == '' || $minigame->server['lastdate'] == null)) {
                            ...
                            if (!empty($new_game_user_record_id)) {
                                $intelligences_games[$key]['server'] = (object) array('status' => 1,
                                                                                    'lastdate' => date("Y-m-d H:i:s"),
                                                                                    'platform' => '');
